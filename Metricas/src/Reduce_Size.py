    ...
    with open(filename, "r", encoding="utf-8") as f:
        ...
        for event in events:
            if (
                ...
                event.get("eventType") == "Reduce_Size"
            ):
...
print(f"Total Reduce_Size: {sum(counts)}")

# Crear gráfico (aunque todo sea 0)
plt.figure()
plt.bar(levels, counts)
plt.xlabel("Level ID")
plt.ylabel("Número de veces (Reduce_Size)")
plt.title("Numero de reducciones por nivel")
plt.xticks(rotation=45)

plt.tight_layout()
plt.savefig("graficos/grafico_reduces.png")